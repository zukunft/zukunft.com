...
/*
  ...
  api/result/index.php - the result API controller: send a calculated result to the frontend
  --------------------
...
$db_con = $app->start_api("result", $msg);
...
if ($db_con->is_open()) {
    ...
    if ($usr->id > 0) {
        ...
        if ($res_id != 0 and $res_id != '') {
            $res = new result($load_usr);
            $res->load_by_id($res_id, $msg);
            $res->load_phrases($msg);
            // do not disclose another user's private/personal result loaded by id (idor); the same
            // neutral message as a missing id, so the response does not confirm the result exists
            if (!$res->is_readable_by($usr)) {
                $msg->add_message_text('result id is missing');
            } else {
                $result = $res->api_json($typ_lst, $msg, $load_usr);
            }
        } else {
            $msg->add_message_text('result id is missing');
        }
