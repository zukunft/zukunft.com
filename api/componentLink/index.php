...
/*
  ...
  api/componentLink/index.php - the component link API controller: send a component link to the frontend
  ---------------------------

  a component link has no name, so unlike the component it can only be requested by its id
...
$db_con = $app->start_api("component_link", $msg);
...
if ($db_con->is_open()) {
    ...
    // of the component link page tabs
    ...
    if ($usr->id > 0) {
        ...
        $lnk = new component_link($load_usr);
        if ($lnk_id > 0) {
            $lnk->load_by_id($lnk_id, $msg);
            $result = $lnk->api_json($typ_lst, $msg, $load_usr);
        } else {
            $msg->add_message_text('component link id is missing');
        }
    }

    // do not disclose another user's private component link loaded by id (idor); the same neutral
