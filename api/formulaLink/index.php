...
/*
  ...
  api/formulaLink/index.php - the formula link API controller: send a formula link to the frontend
  -------------------------

  a formula link has no name, so unlike the formula it can only be requested by its id
...
$db_con = $app->start_api("formula_link", $msg);
...
if ($db_con->is_open()) {
    ...
    if ($usr->id > 0) {
        ...
        $lnk = new formula_link($load_usr);
        if ($lnk_id > 0) {
            $lnk->load_by_id($lnk_id, $msg);
            $result = $lnk->api_json($typ_lst, $msg, $load_usr);
        } else {
            $msg->add_message_text('formula link id is missing');
        }
    }

    // do not disclose another user's private formula link loaded by id (idor); the same neutral
