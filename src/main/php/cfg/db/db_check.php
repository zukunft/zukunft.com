...
// the version 0.0.3 is the first version, which has a build in upgrade process
    function db_upgrade_0_0_3(sql_db $db_con): string
    {
        ...
        $result .= $db_con->change_column_name('user_' . sql_db::TBL_SOURCE, 'comment', sandbox_named::FLD_DESCRIPTION);
        ...
        $result .= $db_con->column_allow_null('user_' . sql_db::TBL_SOURCE, 'source_name');
        $result .= $db_con->column_allow_null('user_' . sql_db::TBL_SOURCE, 'url');
        ...
        $result .= $db_con->column_force_not_null('user_' . sql_db::TBL_SOURCE, user::FLD_ID);
