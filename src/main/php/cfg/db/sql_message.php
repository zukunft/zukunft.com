...
/*
    ...
    model/db/sql_message.php - sql parameters and execution status
    ------------------------
...
class sql_message extends Message
{
     ...
     */
    function clone_reset(): sql_message
    {
        $msg = new sql_message();
        $msg->usr = $this->usr;
        return $msg;
    ...
    function api_array(api_type_list|array $typ_lst = [], user_message $msg = new user_message()): array
    {
        ...
        if ($this->usr != null) {
            $vars[json_fields::USER] = $this->usr->api_json_array($typ_lst, $msg);
        }
        return $vars;
    }
     ...
     * @return void is never expected to fail
     */
    function merge(sql_message|Message $msg_to_add): void
    {
        parent::merge($msg_to_add);
     ...
     * @param sql_message $msg_to_add the user message that contains the db id of an object just added to the db
     * @param string $name the name of the object that is related to the id
     * @return void
     */
    function add_list_name_id(sql_message $msg_to_add, string $name = ''): void
    {
     ...
     * @return void
     */
    function combine_status(sql_message $msg_to_add): void
    {
        if (!$msg_to_add->is_ok()) {
            $this->msg_status = msg_id::NOK;
        }
