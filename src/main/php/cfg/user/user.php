...
class user extends db_id_object_non_sandbox
{
    ...
    // the key of the signup mail stays valid longer, because the user has not asked for it this minute
    const string SIGNUP_KEY_VALIDITY = '+24 hours';
     ...
     * @param string $validity how long the key stays valid e.g. self::SIGNUP_KEY_VALIDITY for the signup mail
     * @return void
     */
    function set_activation_key(string $key, string $validity = self::ACTIVATION_KEY_VALIDITY): void
    {
        $this->activation_key = hash('sha256', $key);
        $timeout = new DateTime();
        try {
            $timeout->modify($validity);
        } catch (Exception $e) {
    ...
    /**
     * verify the credentials; the session of the browser is started by the caller
     * (frontend::start_user_session), like after the signup and the activation
     *
    ...
    function can_set_profile(int $profile_id): bool
    {
        ...
        if ($profile != null) {
            ...
            // the signup user only the profiles of a new account up to the confirmed email
            if ($this->is_signup() and in_array($profile->code_id, user_profiles::SIGNUP_CAN_SET)) {
                $result = true;
            }
            // the admin users can change other users ...
            if ($this->is_admin()) {
    ...
    /**
     * @returns bool true if the user is the system user that creates new users and confirms their email
     */
    function is_signup(): bool
    {
        ...
        $result = false;
        if ($this->is_profile_valid()) {
            if ($this->profile_id == $sys->typ_lst->usr_pro->id(user_profiles::SIGNUP)) {
                $result = true;
            }
        }
        return $result;
    ...
    /**
     * raise the profile of a new account one of user_profiles::SIGNUP_STEPS up e.g. to the confirmed email
     * after the activation link, but never lower it, so that e.g. an admin who resets the password keeps
     * the admin profile; the profile is a critical field, so the requester must be allowed to set it
     *
     * @param string $code_id the profile to reach e.g. user_profiles::EMAIL
     * @param user $usr_req the user who requests the raise, which is the signup system user
     * @param user_message $msg to report a raise that the requester is not allowed to do
     * @return bool true if this user has at least the given profile now
     */
    function raise_signup_profile(string $code_id, user $usr_req, user_message $msg): bool
    {
        global $sys;
        $cur_code_id = $sys->typ_lst->usr_pro->code_id($this->profile_id) ?? user_profiles::IP_ONLY;
        $cur_pos = array_search($cur_code_id, user_profiles::SIGNUP_STEPS);
        ...
        if ($new_pos === false) {
            ...
            if ($usr_req->can_set_profile($new_id)) {
                $this->profile_id = $new_id;
            } else {
                $msg->add(msg_id::USER_NO_UPDATE_PRIVILEGES, [
                    msg_id::VAR_USER_NAME => $this->name() ?? '',
                    msg_id::VAR_USER_PROFILE => $usr_req->name_and_profile() ?? ''
