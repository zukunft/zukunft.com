...
function zuv_lst_get($val_lst_wrd, $word_ids)
{
    ...
    foreach (array_keys($val_lst_wrd) as $val_id) {
        if (!$found) {
            ...
            $wrd_missing = zu_lst_not_in_no_key($word_ids, $val_wrd_ids);
            if (empty($wrd_missing)) {
                // potential result candidate, because the value has all needed words
                log_debug("zuv_lst_get -> can (" . $val_lst[0] . ")") - 10;
                $wrd_extra = zu_lst_not_in_no_key($val_wrd_ids, $word_ids);
                if (empty($wrd_extra)) {
                    // if there is no extra word, it is the correct value
                    log_debug("zuv_lst_get -> is (" . $val_lst[0] . ")");
                    $found = true;
                    $result = array();
                    $result[] = $val_lst[0];
                    $result[] = $val_id;
                    ...
                    log_debug("zuv_lst_get -> is not, because (" . implode(",", $wrd_extra) . ")");
                }
            }
        }
    }

    log_debug("zuv_lst_get -> done (" . implode(",", $result) . ")");
    return $result;
}
...
function zuv_value($val_id, $user_id)
{
    ...
    if ($val_id > 0) {
        ...
        if ($val_id <> $value_del) {
            $result = zu_sql_get1("SELECT user_value FROM user_values WHERE value_id = " . $val_id . " AND user_id = " . $user_id . " AND (excluded is NULL OR excluded = 0);");
            if ($result == NULL) {
                $result = zu_sql_get1("SELECT word_value FROM `values` WHERE value_id = " . $val_id . " AND (excluded is NULL OR excluded = 0);");
            }
        }
    }
    return $result;
...
function zuv_value_table($word_id, $word_lst)
{
...
function zuv_convert($user_value, $user_id)
{
    log_debug('zuv_convert (' . $user_value . ',u' . $user_id . ')');
    $result = $user_value;
...
function zuv_scale($user_value, $value_words, $user_id)
{
    log_debug('zuv_scale (' . $user_value . ',t' . implode(",", $value_words) . ',u' . $user_id . ')');
    $result = $user_value;

    // if it has a scaling word, scale it to one
    if (zut_has_scaling($value_words)) {
        // get any scaling words related to the value
        $scale_word = zut_scale_id($value_words, $user_id);
        log_debug('zuv_scale -> word (' . $scale_word . ')');
        if ($scale_word > 0) {
            $formula_id = zut_formula($scale_word, $user_id);
            if ($formula_id > 0) {
                ...
                if ($formula_text <> "") {
                    ...
                    if (zut_is_type($l_part_wrd_id, DBL_WORD_TYPE_SCALING_HIDDEN)
                        and zut_is_type($r_part_wrd_id, DBL_WORD_TYPE_SCALING)) {
                        ...
                        $result = zuc_math_parse($r_part, $value_words, 0);
                    } else {
...
function zuv_of_wrd_lst($wrd_lst, $user_id)
{
    log_debug("zuv_of_wrd_lst (" . implode(",", $wrd_lst) . ",u" . $user_id . ")");
    ...
    $used_wrd_lst = zut_keep_only_specific($wrd_lst);
    log_debug("zuv_of_wrd_lst -> (" . implode(",", $used_wrd_lst) . ")");
    $result = zu_sql_wrd_ids_val($used_wrd_lst, $user_id);

    log_debug("zuv_of_wrd_lst -> (" . $result['num'] . ")");
    return $result;
}

// similar to zuv_of_wrd_lst but with correct naming and always returns an array to know the user specific result
function zuv_of_wrd_ids($wrd_ids, $user_id)
{
    log_debug("zuv_of_wrd_ids (" . implode(",", $wrd_ids) . ",u" . $user_id . ")");

    // remove the general word if a more specific word is already part of the selection e.g. remove country, if Germany is a selection word
    $used_wrd_ids = zut_keep_only_specific($wrd_ids);
    $result = zu_sql_wrd_ids_val($used_wrd_ids, $user_id);
    return $result;
}

// 
function zuv_of_wrd_lst_scaled($wrd_lst, $user_id)
{
    log_debug("zuv_of_wrd_lst_scaled (" . implode(",", $wrd_lst) . ",u" . $user_id . ")");

    $wrd_val = zuv_of_wrd_lst($wrd_lst, $user_id);

    if ($wrd_val === false) {
        $result = false;
    } else {
        // get the best matching value id related to a word list
        //$val_id = zuv_of_wrd_lst_id($wrd_lst);
        ...
        if ($wrd_val['id'] > 0) {
            log_debug("zuv_of_wrd_lst_scaled -> get word ids " . implode(",", $wrd_lst));
            $val_wrd_ids = zuv_wrd_ids($wrd_val['id'], $user_id);
            $wrd_val['num'] = zuv_scale($wrd_val['num'], $val_wrd_ids, $user_id);
        }
        $result = $wrd_val;
    }
...
// 
function zuv_wrd_group_result($wrd_grp_id, $time_wrd_id, $user_id)
{
    log_debug("zuv_wrd_group_result (" . $wrd_grp_id . ",time" . $time_wrd_id . ",u" . $user_id . ")");
    $result = array();
        ...
        if ($result === false) {
            $sql = "SELECT formula_value_id AS id,
              ...
              WHERE phrase_group_id = " . $wrd_grp_id . "
                AND (user_id = 0 OR user_id IS NULL);";
            $result = zudb_get1($sql, $user_id);
            log_debug("zuv_wrd_group_result -> (" . $result['num'] . ")");
        } else {
            log_debug("zuv_wrd_group_result -> (" . $result['num'] . ")");
        }
    } else {
        log_debug("zuv_wrd_group_result -> (" . $result['num'] . " for " . $user_id . ")");
    }
...
function zuv_frm_related_grp_wrds($wrd_id, $wrd_ids, $user_id)
{
    log_debug("zuv_frm_related_grp_wrds (" . $wrd_id . ",ft" . implode(",", $wrd_ids) . ",u" . $user_id . ")");
    $result = array();

    if ($wrd_id > 0 and !empty($wrd_ids)) {
        // get the relevant values
        $val_ids = zuv_frm_related($wrd_id, $wrd_ids, $user_id);
        ...
        $result = zuv_frm_related_grp_wrds_part($val_ids, $wrd_id, $wrd_ids, $user_id);
    }

    log_debug("zuv_frm_related_grp_wrds -> (" . zu_lst_dsp($result) . ")");
    return $result;
}

// similar to zuv_frm_related_grp_wrds, but for calculated values (therefore the prefic is VC for ValuesCalculated)
function zuvc_frm_related_grp_wrds($val_wrd_lst, $wrd_id, $frm_ids, $user_id)
{
    log_debug("zuvc_frm_related_grp_wrds (vt" . implode(",", $val_wrd_lst) . "," . $wrd_id . ",f" . implode(",", $frm_ids) . ",u" . $user_id . ")");
    $result = array();

    if ($wrd_id > 0 and !empty($frm_ids)) {
        // get the relevant values
        $valc_ids = zuvc_frm_related($frm_ids, $wrd_id, $user_id);

        // get the word groups for which a formula result is expected
        // maybe exclude word groups already here where not all needed values for the formula are in the database
        $result = zuvc_frm_related_grp_wrds_part($valc_ids, $wrd_id, $frm_ids, $user_id);
    }

    log_debug("zuvc_frm_related_grp_wrds -> (" . zu_lst_dsp($result) . ")");
    return $result;
}
...
function zuv_frm_related($wrd_id, $wrd_ids, $user_id)
{
    log_debug("zuv_frm_related (" . $wrd_id . ",ft" . implode(",", $wrd_ids) . ",u" . $user_id . ")");
    $result = array();
    ...
    if ($wrd_id > 0 and !empty($wrd_ids)) {
        ...
        $result = zu_sql_get_ids($sql);
    }

    log_debug("zuv_frm_related -> (" . implode(",", $result) . ")");
    return $result;
}

function zuvc_frm_related($frm_ids, $wrd_id, $user_id)
{
    log_debug("zuvc_frm_related (f" . implode(",", $frm_ids) . ",u" . $user_id . ")");
    $result = array();
    ...
    log_debug("zuvc_frm_related -> (" . implode(",", $result) . ")");
    return $result;
}

// group words
// kind of similar to zu_sql_val_lst_wrd
function zuv_frm_related_grp_wrds_part($val_ids, $wrd_id, $wrd_ids, $user_id)
{
    log_debug("zuv_frm_related_grp_wrds_part (v" . implode(",", $val_ids) . ",t" . $wrd_id . ",ft" . implode(",", $wrd_ids) . ",u" . $user_id . ")");
    $result = array();
    ...
    if ($wrd_id > 0 and !empty($wrd_ids) and !empty($val_ids)) {
        ...
        while ($val_entry = mysqli_fetch_array($sql_result, MySQLi_NUM)) {
            if ($value_id == $val_entry[0]) {
                $wrd_result[] = $val_entry[2];
                //zu_debug('zu_sql_val_lst_wrd -> add word '.$val_entry[2].' to ('.$value_id.')');
            } else {
                if ($value_id >= 0) {
    ...
    log_debug("zuv_frm_related_grp_wrds_part -> (" . zu_lst_dsp($result) . ")");
    return $result;
}

// similar to zuv_frm_related_grp_wrds_part, but for formula result values
function zuvc_frm_related_grp_wrds_part($frm_val_ids, $wrd_id, $wrd_ids, $user_id)
{
    log_debug("zuvc_frm_related_grp_wrds_part (v" . implode(",", $frm_val_ids) . ",t" . $wrd_id . ",ft" . implode(",", $wrd_ids) . ",u" . $user_id . ")");
    $result = array();

    if ($wrd_id > 0 and !empty($wrd_ids) and !empty($frm_val_ids)) {
        $time_wrd_id = zut_time_id($wrd_ids, $user_id);
        $wrd_ids = zut_ids_ex_time($wrd_ids, $user_id);
        $wrd_ids[] = $wrd_id; // add the main word to the exclude words
        if ($time_wrd_id > 0) {
    ...
    log_debug("zuvc_frm_related_grp_wrds_part -> (" . zu_lst_dsp($result) . ")");
    return $result;
}
...
{
    log_debug("zuv_ids_wrd_lst (" . implode(",", $val_ids) . ",u" . $user_id . ")");
    $result = array();
    ...
    log_debug("zuv_ids_wrd_lst -> (" . implode(",", $result) . " for " . implode(",", $val_ids) . ")");
    return $result;
}

// same as zuv_ids_wrd_lst, but includes the category words e.g. "Year" for "2016"
function zuv_ids_wrd_lst_incl_cat($val_ids, $user_id)
{
    log_debug("zuv_ids_wrd_lst_incl_cat (" . implode(",", $val_ids) . ",u" . $user_id . ")");
    $wrd_lst = zuv_ids_wrd_lst($val_ids, $user_id);

    foreach (array_keys($wrd_lst) as $wrd_id) {
        $wrd_cat_lst = zut_foaf_parent($wrd_id);
    ...
    log_debug("zuv_ids_wrd_lst_incl_cat -> (the words " . implode(",", $wrd_lst) . " are related to the values " . implode(",", $val_ids) . ")");
    return $wrd_lst;
}
