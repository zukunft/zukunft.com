...
function zuu_is_admin($user_id)
{
    ...
    if ($user_profile == clo(DBL_USER_ADMIN)) {
        $result = true;
    }
    return $result;
}
...
function zuu_dsp_sandbox_wrd($user_id, $back_link)
{
    ...
    log_debug('zuu_dsp_sandbox_wrd -> done');
    return $result;
}

// display formula changes by the user which are not (yet) standard 
function zuu_dsp_sandbox_frm($user_id, $back_link)
{
    log_debug('zuu_dsp_sandbox_frm(u' . $user_id . ')');
    $result = ''; // reset the html code var
    ...
    log_debug('zuu_dsp_sandbox_frm -> done');
    return $result;
}

// display value changes by the user which are not (yet) standard 
function zuu_dsp_sandbox_val($user_id, $back_link)
{
    log_debug('zuu_dsp_sandbox_val(u' . $user_id . ')');
    $result = ''; // reset the html code var
    ...
    while ($wrd_row = mysqli_fetch_array($sql_result, MySQLi_NUM)) {
        ...
        $usr_val_lst = zu_sql_get_lst($sql_usr_val);
        ...
        $wrd_lst = zuv_wrd_lst($wrd_row[2], $user_id);
        $result .= '<td>' . zut_dsp_lst_txt($wrd_lst) . '</td>';
        $result .= '</tr>';
    }
    $result .= '</table>';

    log_debug('zuu_dsp_sandbox_val -> done');
    return $result;
}

// display changes by the user which are not (yet) standard 
function zuu_dsp_sandbox($user_id, $back_link)
{
    log_debug('zuu_dsp_sandbox(u' . $user_id . ',b' . $back_link . ')');
    $result = zuu_dsp_sandbox_val($user_id, $back_link);
    $result .= zuu_dsp_sandbox_frm($user_id, $back_link);
    $result .= zuu_dsp_sandbox_wrd($user_id, $back_link);
    return $result;
}

// display the latest changes by the user
function zuu_dsp_changes($user_id, $back_link)
{
    log_debug('zuu_dsp_changes (u' . $user_id . ',b' . $back_link . ')');
    $result = ''; // reset the html code var
    ...
    while ($wrd_row = mysqli_fetch_array($sql_result, MySQLi_NUM)) {
        ...
        if ($wrd_row[4] == "value") {
            if ($wrd_row[5] > 0) {
                $wrd_lst = zuv_wrd_lst($wrd_row[5], $user_id);
    ...
    log_debug('zuu_dsp_changes -> done');
    return $result;
}
