...
function zuv_owner($val_id) {
  log_debug('zuv_owner (v'.$val_id.')');
  $user_id = zu_sql_get_value("values", "user_id", "value_id", $val_id);  
  return $user_id;
}
...
function zuv_changer($val_id) {
  log_debug('zuv_changer (v'.$val_id.')');
  
  $sql = "SELECT user_id 
            FROM user_values 
           WHERE value_id = ".$val_id."
             AND (excluded <> 1 OR excluded is NULL)";
  $user_id = zu_sql_get1($sql);
  return $user_id;
}

// true if the user is the owner and noone else has changed the value
function zuv_can_change($val_id, $user_id) {
  log_debug('zuv_can_change (v'.$val_id.',u'.$user_id.')');
  $can_change = false;
  $val_owner = zuv_owner($val_id);
  if ($val_owner == $user_id OR $val_owner <= 0) {
    $val_user = zuv_changer($val_id);
    if ($val_user == $user_id OR $val_user <= 0) {
      $can_change = true;
    }  
  }  

  log_debug('zuv_can_change -> ('.zu_dsp_bool($can_change).')');
  return $can_change;
}
...
function zuv_db_add($new_value, $wrd_ids, $user_id) {
  ...
  $log_id = zu_log($user_id, "add", "values", "word_value", "", $new_value, 0);
  if ($log_id > 0) {
    // insert the value
    ...
    if ($result) {
      ...
      $result = zu_log_upd($log_id, $val_id, $user_id);
      if ($val_id > 0 and $result) {
        // link the words
        foreach ($wrd_ids as $wrd_id) {
          if ($wrd_id > 0 and $result) {
            $result = zuvt_db_add($val_id, $wrd_id, $user_id);
          }
        }
      }  
    }
...
function zuvt_db_add($val_id, $wrd_id, $user_id) {
  log_debug("zuvt_db_add (v".$val_id.",t".$wrd_id.",u".$user_id.")");
  $result = false;

  if (zuv_can_change($val_id, $user_id)) {
    // log the insert attempt first
    $new_text = zut_name($wrd_id, $user_id);
...
function zuv_db_upd($val_id, $new_value, $user_id) {
  ...
  $old_value = zuv_value($val_id, $user_id);
  ...
  if ($old_value <> $new_value) {
    if (zuv_can_change($val_id, $user_id)) {
      ...
      if (zu_log($user_id, "update", "user_values", "word_value", $old_value, $new_value, $val_id) > 0 ) {
        $sql = "SELECT value_id FROM `user_values` WHERE value_id = ".$val_id." AND user_id = ".$user_id.";";
        $user_value_id = zu_sql_get($sql);
        if ($user_value_id <= 0) {
  ...
  log_debug("zuv_db_upd -> done(".$result.")");
  return $result;
}
...
function zuvt_db_upd($link_id, $val_id, $wrd_new_id, $user_id) {
  ...
  if (zu_log_link_ref($user_id, "update", "value_phrase_links", 
                      $val_id, 0, $wrd_old_id, 
                      $val_id, 0, $wrd_new_id, $link_id, "word") > 0 ) {
    ...
    if ($link_id > 0) {
      ...
      if ($link_id > 0) {
        ...
        log_warning("Dublicate words (".$wrd_old_id.") for value ".$val_id." found, but they have been removed automatically.","zuvt_db_upd");
      }  
    }
    $sql_result = mysqli_query($sql);
  }      
  log_debug("zuvt_db_upd ... done");
}

// add the source of a new value
function zuvs_db_add($val_id, $src_id, $user_id) {
  log_debug("zuvs_db_add (".$val_id.",s".$src_id.",u".$user_id.")");
  $result = false;
...
function zuvs_db_upd($val_id, $src_id, $user_id) {
  ...
  // if the user is the owner and no other user has adjusted the value, really delete the value in the database
  $old_id = zuv_source($val_id);
  if (zuv_can_change($val_id, $user_id) OR $old_id <= 0) {
    if ($old_id <> $src_id) {
...
function zuv_db_usr_check ($val_id, $user_id) {
  log_debug("zuv_db_usr_check (v".$val_id.",u".$user_id.")");
  $result = false;
  ...
  if (zu_log($user_id, "del", "values", "word_value", $old_value, "", $val_id) > 0 ) {
    ...
    if ($all_excluded == 1) {
      if ($user_excluded == 1) {
        ...
        log_warning("Looks like database cleanup for value ".$val_id." has not yet been done.", "zuv_db_del");
      }
    } else {
      if ($user_excluded == 1) {
        // this case is not expected if the database is clean
        log_warning("Looks like value ".$val_id." has been displayed to the user ".$user_id.", but the user has switched it off already.", "zuv_db_del");
      } else {  
        // if the user is the owner and no other user has adjusted the value, really delete the value in the database
        if (zuv_can_change($val_id, $user_id)) {
...
function zuv_db_user_del($val_id, $user_id) {
...
/*
// change a link of a word to a value
function zuvt_db_del($val_id, $wrd_id, $user_id) {
  zu_debug("zuvt_db_del (v".$val_id.",t".$wrd_id.",u".$user_id.")");   
  $result = false;
