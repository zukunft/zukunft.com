    ...
    */
        ...
        if ($this->id > 0) {
            ...
            if ($this->id > 0) {
                ...
                if ($this->owner_id > 0) {
                    ...
                    $sql_set = "UPDATE `values` SET user_id = " . $this->usr->id . " WHERE value_time_series_id = " . $this->id . ";";
                    $sql_result = $db_con->exe($sql_set, DBL_SYSLOG_ERROR, "value_time_series->load_standard", (new Exception)->getTraceAsString(), $debug - 10);
    ...
    function load_rec($sql_where, $debug)
    {
      ...
      LEFT JOIN user_values u ON u.value_time_series_id = v.value_time_series_id 
        ...
        log_debug('value_time_series->load_rec -> sql "' . $sql . '"', $debug - 18);
        $db_val = $db_con->get1($sql, $debug - 5);
        $this->row_mapper($db_val, true);
        if ($this->id > 0) {
            log_debug('value_time_series->load_rec -> got ' . $this->number . ' with id ' . $this->id, $debug - 14);
        }
    }
