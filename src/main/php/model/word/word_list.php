        ...
        if ($sql_where == '') {
            ...
            if (SQL_DB_TYPE == DB_TYPE_POSTGRES) {
                $sql = "SELECT t.word_id,
                    ...
                    CASE WHEN (u.word_type_id <> '' IS NOT TRUE) THEN t.word_type_id ELSE u.word_type_id END AS word_type_id,
                    CASE WHEN (u.excluded     <> '' IS NOT TRUE) THEN t.excluded     ELSE u.excluded     END AS excluded,
                    t.values
                FROM words t 
    ...
    function are_and_contains($debug)
    {
        ...
        if (count($added_lst->lst) > 0) {
            ...
            do {
                ...
                if (count($added_lst->lst) > 0) {
                    log_debug('word_list->are_and_contains -> add ' . $added_lst->name() . ' to ' . $wrd_lst->name(), $debug - 18);
                }
    ...
    function differentiators_all($debug)
    {
        ...
        if (count($added_lst->lst) > 0) {
            ...
            do {
                ...
                $added_lst->diff($wrd_lst, $debug - 20);
                if (count($added_lst->lst) > 0) {
                    log_debug('word_list->differentiators -> add ' . $added_lst->name() . ' to ' . $wrd_lst->name(), $debug - 8);
                }
                $wrd_lst->merge($added_lst, $debug - 20);
                $loops++;
            } while (count($added_lst->lst) > 0 and $loops < MAX_LOOP);
