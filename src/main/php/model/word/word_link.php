...
class word_link
{
    // TODO use the user sandbox as base
        ...
        if ($sql_where == '') {
            ...
            if ($this->id > 0) {
                ...
                if ($new_name <> $this->name) {
                    $db_con->set_type(DB_TYPE_WORD_LINK);
                    $db_con->update($this->id, 'word_link_name', $new_name, $debug - 1);
                    $this->name = $new_name;
                }
            }
    ...
    */
    ...
    function dsp_add($back, $debug)
    {
        ...
        if (isset($this->verb)) {
            $result .= $this->verb->dsp_selector('both', $form_name, "col-sm-6", $back, $debug - 1);
        }
        if (isset($this->to)) {
            $result .= $this->to->dsp_selector(0, $form_name, 0, "col-sm-6", $back, $debug - 1);
        }
        $result .= '</div>';
        $result .= '<br>';
    ...
    function dsp_edit($back, $debug)
    {
        ...
        if ($this->id > 0) {
            ...
            $result .= '<div class="form-row">';
            if (isset($this->from)) {
                $result .= $this->from->dsp_selector(0, $form_name, 1, "col-sm-4", $back, $debug - 1);
            }
            if (isset($this->verb)) {
                $result .= $this->verb->dsp_selector('forward', $form_name, "col-sm-4", $back, $debug - 1);
            }
            if (isset($this->to)) {
                $result .= $this->to->dsp_selector(0, $form_name, 2, "col-sm-4", $back, $debug - 1);
            }
            $result .= '</div>';
            $result .= dsp_form_end('', $back);
            $result .= '<br>';
    ...
    */
    ...
    private function add($db_con, $debug)
    {
        ...
        if ($log->id > 0) {
            ...
            //array($this->from_id, $this->verb_id, $this->to_id, $this->usr->id), $debug - 1);
            if ($this->id > 0) {
