...
class word {
  ...
  private function load_standard($debug) {
    ...
    if ($sql_where == '') {
      $result .= log_err("ID missing to load the standard word.", "word->load_standard", '', (new Exception)->getTraceAsString(), $this->usr);
    } else{  
      $sql = "SELECT word_id,
                     user_id,
                     word_name,
  ...
  function load($debug) {
    ...
    if (!isset($this->usr)) {
      ...
      if ($sql_where == '') {
        log_err("Internal error in the where clause.", "word->load", '', (new Exception)->getTraceAsString(), $this->usr);
      } else{  
  ...
  function are_and_contains ($debug) {
    ...
    if (count($added_lst->lst) > 0) {
      ...
      do {
        ...
        if (count($added_lst->lst) > 0) { log_debug('word->are_and_contains -> add '.$added_lst->name().' to '.$wrd_lst->name(), $debug-18); }
