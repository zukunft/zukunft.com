...
class formula extends user_sandbox
{
        ...
        if (!isset($this->usr)) {
            ...
            if ($db_con->get_where() <> '') {
                ...
                if ($db_frm['formula_id'] <= 0) {
                    $this->id = 0;
                    // TODO check that a failed load always just set the id to 0 and that this is always used for checking if the object is saved
                    // $this->reset();
                } else {
    ...
    function to_num($phr_lst, $back, $debug)
    {
        ...
        if (isset($time_wrd)) {
            $time_phr = $time_wrd->phrase($debug - 1);
        }
      ...
      if ($this->has_verb ($this->ref_text, $this->usr->id, $debug-1)) {
        ...
        $frm_wrd_ids = $this->wrd_ids($this->ref_text, $this->usr->id, $debug-1);
            ...
            if (isset($fv_add_phr_lst)) {
                log_debug('formula->calc -> use words ' . $fv_add_phr_lst->dsp_id() . ' for the result', $debug - 12);
            }
            ...
            if (isset($fv_add_phr_lst)) {
                log_debug('formula->calc -> ' . count($fv_lst->lst) . ' formula results to save', $debug - 8);
            }
  ...
  */
    ...
    */
    ...
    function dsp_edit($add, $wrd, $back, $debug)
    {
        ...
        if (!$this->is_special($debug - 1) or $this->usr->is_admin($debug - 1)) {
        ...
        if ($this->id > 0) {
            ...
            // allow to test and refresh the formula and show some sample values
            $numbers_html = $this->dsp_test_and_samples($back, $debug);
  ...
  */
    ...
    function element_refresh($frm_text, $debug)
    {
        ...
        if (str_replace('1', '', $result) == '') {
            $result .= $this->element_refresh_type($frm_text, DBL_FORMULA_PART_TYPE_WORD, 0, $this->usr->id, $debug - 1);
        }
        // update formula links of the standard formula
        if (str_replace('1', '', $result) == '') {
            $result .= $this->element_refresh_type($frm_text, DBL_FORMULA_PART_TYPE_FORMULA, 0, $this->usr->id, $debug - 1);
        }
        ...
        foreach ($db_lst as $db_row) {
            ...
            if (str_replace('1', '', $result) == '') {
                $result .= $this->element_refresh_type($frm_text, DBL_FORMULA_PART_TYPE_WORD, $db_row['user_id'], $this->usr->id, $debug - 1);
            }
            // update formula links of the standard formula
            if (str_replace('1', '', $result) == '') {
                $result .= $this->element_refresh_type($frm_text, DBL_FORMULA_PART_TYPE_FORMULA, $db_row['user_id'], $this->usr->id, $debug - 1);
            }
        }
