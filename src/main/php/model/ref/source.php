...
class source {
  ...
  private function load_standard($debug) {
    ...
    if ($sql_where == '') {
      $result .= log_err("ID missing to load the standard source.", "source->load_standard", '', (new Exception)->getTraceAsString(), $this->usr);
    } else{  
  ...
  function load($debug) {
    ...
    if (!isset($this->usr)) {
      ...
      if ($sql_where == '') {
        log_err("Internal error on the where clause.", "source->load", '', (new Exception)->getTraceAsString(), $this->usr);
      } else{  
        $sql = "SELECT s.source_id,
