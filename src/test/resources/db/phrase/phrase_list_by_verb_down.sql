PREPARE phrase_list_by_verb_down (bigint, bigint) AS
    SELECT     s.phrase_id,
     ...
     LEFT JOIN user_phrases u ON s.phrase_id = u.phrase_id
                             AND u.user_id = $1
         WHERE s.phrase_id IN (SELECT from_phrase_id FROM triples WHERE verb_id = $2
           AND COALESCE(excluded, 0) = 0) ORDER BY s.usage DESC, phrase_name;