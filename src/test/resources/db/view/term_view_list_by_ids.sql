PREPARE term_view_list_by_ids (bigint, bigint[]) AS
    SELECT s.term_view_id,
      ...
      FROM term_views s
 LEFT JOIN user_term_views u ON s.term_view_id =  u.term_view_id AND  u.user_id = $1
 LEFT JOIN views l           ON s.view_id      =  l.view_id
 LEFT JOIN user_views ul     ON l.view_id      = ul.view_id      AND ul.user_id = $1
 LEFT JOIN terms l2          ON s.term_id      = l2.term_id
 LEFT JOIN user_terms ul2    ON l2.term_id     = ul2.term_id     AND ul2.user_id = $1
