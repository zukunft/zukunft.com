PREPARE word_link_list_by_phr (int, int) AS
    SELECT s.word_link_id,
           ...
           CASE WHEN (ul.name          <> '' IS NOT TRUE) THEN l.name           ELSE ul.name           END AS name1,
           ...
           CASE WHEN (ul2.name         <> '' IS NOT TRUE) THEN l2.name          ELSE ul2.name          END AS name2,
