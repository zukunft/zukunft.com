PREPARE triple_list_by_phr_lst_down_and_vrb (bigint, bigint[], bigint) AS
    SELECT     s.triple_id,
               ...
               CASE WHEN (ul2.usage               IS     NULL) THEN l2.usage         ELSE ul2.usage        END AS usage2,
               CASE WHEN (ul2.impact              IS     NULL) THEN l2.impact        ELSE ul2.impact       END AS impact2,
               CASE WHEN (ul2.excluded            IS     NULL) THEN l2.excluded      ELSE ul2.excluded     END AS excluded2,
               CASE WHEN (ul2.share_type_id       IS     NULL) THEN l2.share_type_id ELSE ul2.share_type_idEND AS share_type_id2,
     ...
     LEFT JOIN user_triples u ON s.triple_id = u.triple_id
                             ...
                             AND ul2.user_id = $1
         WHERE s.to_phrase_id = ANY ($2) AND s.verb_id = $3 ORDER BY s.verb_id, triple_name, s.triple_id;