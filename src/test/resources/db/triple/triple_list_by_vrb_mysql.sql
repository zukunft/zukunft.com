PREPARE triple_list_by_vrb FROM
   'SELECT     s.triple_id,
               ...
               IF(ul2.`usage`       IS NULL, l2.`usage`,       ul2.`usage`)       AS usage2,
               IF(ul2.impact        IS NULL, l2.impact,        ul2.impact)        AS impact2,
     ...
     LEFT JOIN user_triples u ON s.triple_id = u.triple_id
                             AND u.user_id = ? LEFT JOIN phrases l ON s.from_phrase_id = l.phrase_id LEFT JOIN user_phrases ul ON l.phrase_id = ul.phrase_id
                             AND ul.user_id = ? LEFT JOIN phrases l2 ON s.to_phrase_id = l2.phrase_id LEFT JOIN user_phrases ul2 ON l2.phrase_id = ul2.phrase_id
                             AND ul2.user_id = ?
         WHERE s.verb_id = ? ORDER BY s.verb_id, triple_name, s.triple_id LIMIT ? OFFSET ?';