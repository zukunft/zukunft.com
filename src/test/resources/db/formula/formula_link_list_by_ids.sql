PREPARE formula_link_list_by_ids (bigint, bigint[]) AS
    SELECT     s.formula_link_id,
               ...
               CASE WHEN (ul2.formula_name <> ''  IS NOT TRUE) THEN l2.formula_name        ELSE ul2.formula_name       END AS formula_name2
          FROM formula_links s
     LEFT JOIN user_formula_links u ON s.formula_link_id = u.formula_link_id
                                   AND u.user_id = $1 LEFT JOIN phrases l ON s.phrase_id = l.phrase_id LEFT JOIN user_phrases ul ON l.phrase_id = ul.phrase_id
                                   AND ul.user_id = $1 LEFT JOIN formulas l2 ON s.formula_id = l2.formula_id LEFT JOIN user_formulas ul2 ON l2.formula_id = ul2.formula_id
                                   AND ul2.user_id = $1
         WHERE s.formula_link_id = ANY ($2);