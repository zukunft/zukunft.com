PREPARE value_list_by_phr_lst_p2 (bigint, bigint, text, text, bigint) AS
    SELECT     '' AS group_id,
               '' AS user_group_id,
               phrase_id_1,
               phrase_id_2,
  ...
  UNION SELECT group_id,
         ...
         WHERE group_id ilike $3
           AND group_id ilike $4
  ...
  UNION SELECT s.group_id,
     ...
     LEFT JOIN user_values u ON s.group_id = u.group_id
                            AND u.user_id = $5
         WHERE s.group_id ilike $3
           AND s.group_id ilike $4
