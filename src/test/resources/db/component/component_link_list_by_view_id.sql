PREPARE component_link_list_by_view_id (bigint, bigint) AS
    SELECT     s.component_link_id,
               ...
               CASE WHEN (ul2.component_type_id IS     NULL) THEN l2.component_type_id ELSE ul2.component_type_idEND AS component_type_id2,
     ...
     LEFT JOIN user_component_links u ON s.component_link_id = u.component_link_id
                                     AND u.user_id = $1 LEFT JOIN components l ON s.component_id = l.component_id LEFT JOIN user_components ul ON l.component_id = ul.component_id
                                     AND ul.user_id = $1 LEFT JOIN components l2 ON s.component_id = l2.component_id LEFT JOIN user_components ul2 ON l2.component_id = ul2.component_id
                                     AND ul2.user_id = $1
         WHERE s.view_id = $2 ORDER BY s.order_nbr;