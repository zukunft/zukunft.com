PREPARE component_list_by_view_id FROM
   'SELECT     s.component_id,
               ...
               IF(u.linked_component_id    IS NULL, s.linked_component_id,    u.linked_component_id)    AS linked_component_id,
               IF(u.component_link_type_id IS NULL, s.component_link_type_id, u.component_link_type_id) AS component_link_type_id,
     ...
     LEFT JOIN user_components u ON s.component_id = u.component_id
                                AND u.user_id = ? LEFT JOIN component_links l ON s.component_id = l.component_id
         WHERE l.view_id = ? ORDER BY l.order_nbr';