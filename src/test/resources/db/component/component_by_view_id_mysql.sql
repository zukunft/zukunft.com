PREPARE component_by_view_id FROM
   'SELECT     s.component_link_id,
               ...
               IF(ul.component_name          IS NULL, l.component_name,          ul.component_name)          AS component_name,
     ...
     LEFT JOIN user_component_links u ON s.component_link_id = u.component_link_id
                                     AND u.user_id = ? LEFT JOIN components l ON s.component_id = l.component_id LEFT JOIN user_components ul ON l.component_id = ul.component_id
                                     AND ul.user_id = ? LEFT JOIN components l2 ON s.component_id = l2.component_id LEFT JOIN user_components ul2 ON l2.component_id = ul2.component_id
                                     AND ul2.user_id = ?
         WHERE s.view_id = ? ORDER BY s.order_nbr';