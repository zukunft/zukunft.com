PREPARE component_list_by_view_id (bigint, bigint) AS
    SELECT     s.component_id,
               ...
               CASE WHEN (u.linked_component_id    IS     NULL) THEN s.linked_component_id    ELSE u.linked_component_id    END AS linked_component_id,
               CASE WHEN (u.component_link_type_id IS     NULL) THEN s.component_link_type_id ELSE u.component_link_type_id END AS component_link_type_id,
     ...
     LEFT JOIN user_components u ON s.component_id = u.component_id
                                AND u.user_id = $1 LEFT JOIN component_links l ON s.component_id = l.component_id
         WHERE l.view_id = $2 ORDER BY l.order_nbr;