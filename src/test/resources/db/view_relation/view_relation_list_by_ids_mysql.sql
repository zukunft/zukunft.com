PREPARE view_relation_list_by_ids FROM
   'SELECT     s.view_relation_id,
     ...
     LEFT JOIN user_view_relations u ON s.view_relation_id = u.view_relation_id
                                    AND u.user_id = ? LEFT JOIN views l ON s.parent_view_id = l.view_id LEFT JOIN user_views ul ON l.view_id = ul.view_id
                                    AND ul.user_id = ? LEFT JOIN views l2 ON s.child_view_id = l2.view_id LEFT JOIN user_views ul2 ON l2.view_id = ul2.view_id
                                    AND ul2.user_id = ?
