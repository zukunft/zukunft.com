...
function run_word_test()
{
    ...
    // check loading a word by name and id works by loading the word first by name and then by id, which should return the same name
    $wrd1 = new word;
    $wrd1->name = word::TEST_NAME_READ;
    $wrd1->usr = $usr1;
    $wrd1->load();
    $wrd2 = new word;
    $wrd2->id = $wrd1->id;
    ...
    // display
    $back = 1;
    $target = '<a href="/http/view.php?words=' . $wrd_company->id . '&back=1">' . word::TEST_NAME_READ . '</a>';
    $result = $wrd_company->display($back);
    test_dsp('word->display "' . word::TEST_NAME_READ . '"', $target, $result);
    ...
    test_dsp('word->children for "' . word::TEST_NAME_READ . '"', $target, $result, TIMEOUT_LIMIT_DB, 'out of ' . $wrd_lst->dsp_id() . '');
    ...
    test_dsp('word->children for "' . word::TEST_NAME_READ . '" excluding the start word', $target, $result, TIMEOUT_LIMIT, 'out of ' . $wrd_lst->dsp_id() . '');
    ...
    test_dsp('word->are for "' . word::TEST_NAME_READ . '"', $target, $result, TIMEOUT_LIMIT, 'out of ' . $wrd_lst->dsp_id() . '');
    ...
    test_dsp('word->are for "' . word::TEST_NAME_READ . '" including the start word', $target, $result, TIMEOUT_LIMIT, 'out of ' . $wrd_lst->dsp_id() . '');
    ...
    $wrd_add = new word;
    $wrd_add->name = word::TEST_NAME_ADD;
    $wrd_add->usr = $usr1;
    ...
    $target = 'zukunft.com system batch job added ' . word::TEST_NAME_ADD . '';
    test_dsp('word->save logged for "' . word::TEST_NAME_ADD . '"', $target, $result);

    // ... test if the new word has been created
    $wrd_added = load_word(word::TEST_NAME_ADD);
    $wrd_added->load();
    if ($wrd_added->id > 0) {
        $result = $wrd_added->name;
    }
    $target = word::TEST_NAME_ADD;
    test_dsp('word->load of added word "' . word::TEST_NAME_ADD . '"', $target, $result);

    // check if the word can be renamed
    $wrd_added->name = word::TEST_NAME_CHANGED;
    $result = num2bool($wrd_added->save());
    $target = true;
    test_dsp('word->save rename "' . word::TEST_NAME_ADD . '" to "' . word::TEST_NAME_CHANGED . '".', $target, $result, TIMEOUT_LIMIT_DB);

    // check if the word renaming was successful
    $wrd_renamed = new word;
    $wrd_renamed->name = word::TEST_NAME_CHANGED;
    ...
    test_dsp('word->load renamed word "' . word::TEST_NAME_CHANGED . '"', $target, $result);
    ...
    $log->usr = $usr1;
    $result = $log->dsp_last(true);
    $target = 'zukunft.com system batch job changed ' . word::TEST_NAME_ADD . ' to ' . word::TEST_NAME_CHANGED . '';
    test_dsp('word->save rename logged for "' . word::TEST_NAME_CHANGED . '"', $target, $result);

    // check if the word parameters can be added
    $wrd_renamed->plural = word::TEST_NAME_CHANGED . 's';
    $wrd_renamed->description = word::TEST_NAME_CHANGED . ' description';
    $wrd_renamed->type_id = cl(db_cl::WORD_TYPE, word_type_list::DBL_OTHER);
    $result = num2bool($wrd_renamed->save());
    $target = true;
    test_dsp('word->save all word fields beside the name for "' . word::TEST_NAME_CHANGED . '"', $target, $result, TIMEOUT_LIMIT_DB_MULTI);

    // check if the word parameters have been added
    $wrd_reloaded = load_word(word::TEST_NAME_CHANGED);
    $result = $wrd_reloaded->plural;
    $target = word::TEST_NAME_CHANGED . 's';
    test_dsp('word->load plural for "' . word::TEST_NAME_CHANGED . '"', $target, $result);
    $result = $wrd_reloaded->description;
    $target = word::TEST_NAME_CHANGED . ' description';
    test_dsp('word->load description for "' . word::TEST_NAME_CHANGED . '"', $target, $result);
    $result = $wrd_reloaded->type_id;
    $target = cl(db_cl::WORD_TYPE, word_type_list::DBL_OTHER);
    test_dsp('word->load type_id for "' . word::TEST_NAME_CHANGED . '"', $target, $result);
    ...
    $log->usr = $usr1;
    $result = $log->dsp_last(true);
    $target = 'zukunft.com system batch job added ' . word::TEST_NAME_CHANGED . 's';
    test_dsp('word->load plural for "' . word::TEST_NAME_CHANGED . '" logged', $target, $result);
    $log->field = 'description';
    $result = $log->dsp_last(true);
    $target = 'zukunft.com system batch job added ' . word::TEST_NAME_CHANGED . ' description';
    test_dsp('word->load description for "' . word::TEST_NAME_CHANGED . '" logged', $target, $result);
    test_dsp('word->load ref_2 for "' . word::TEST_NAME_CHANGED . '" logged', $target, $result);
    $log->field = 'word_type_id';
    $result = $log->dsp_last(true);
    $target = 'zukunft.com system batch job added differentiator filler';
    test_dsp('word->load type_id for "' . word::TEST_NAME_CHANGED . '" logged', $target, $result);

    // check if a user specific word is created if another user changes the word
    $wrd_usr2 = new word;
    $wrd_usr2->name = word::TEST_NAME_CHANGED;
    $wrd_usr2->usr = $usr2;
    $wrd_usr2->load();
    $wrd_usr2->plural = word::TEST_NAME_CHANGED . 's2';
    $wrd_usr2->description = word::TEST_NAME_CHANGED . ' description2';
    $wrd_usr2->type_id = cl(db_cl::WORD_TYPE, word_type_list::DBL_TIME);
    $result = num2bool($wrd_usr2->save());
    $target = true;
    test_dsp('word->save all word fields for user 2 beside the name for "' . word::TEST_NAME_CHANGED . '"', $target, $result, TIMEOUT_LIMIT_DB_MULTI);

    // check if a user specific word changes have been saved
    $wrd_usr2_reloaded = new word;
    $wrd_usr2_reloaded->name = word::TEST_NAME_CHANGED;
    $wrd_usr2_reloaded->usr = $usr2;
    $wrd_usr2_reloaded->load();
    $result = $wrd_usr2_reloaded->plural;
    $target = word::TEST_NAME_CHANGED . 's2';
    test_dsp('word->load plural for "' . word::TEST_NAME_CHANGED . '"', $target, $result);
    $result = $wrd_usr2_reloaded->description;
    $target = word::TEST_NAME_CHANGED . ' description2';
    test_dsp('word->load description for "' . word::TEST_NAME_CHANGED . '"', $target, $result);
    ...
    $result = $wrd_reloaded->plural;
    $target = word::TEST_NAME_CHANGED . 's';
    test_dsp('word->load plural for "' . word::TEST_NAME_CHANGED . '" unchanged for user 1', $target, $result);
    $result = $wrd_reloaded->description;
    $target = word::TEST_NAME_CHANGED . ' description';
    test_dsp('word->load description for "' . word::TEST_NAME_CHANGED . '" unchanged for user 1', $target, $result);
    $result = $wrd_reloaded->type_id;
    $target = cl(db_cl::WORD_TYPE, word_type_list::DBL_OTHER);
    test_dsp('word->load type_id for "' . word::TEST_NAME_CHANGED . '" unchanged for user 1', $target, $result);
    ...
    $wrd_usr2->usr = $usr2;
    $wrd_usr2->load();
    $wrd_usr2->plural = word::TEST_NAME_CHANGED . 's';
    $wrd_usr2->description = word::TEST_NAME_CHANGED . ' description';
    $wrd_usr2->type_id = cl(db_cl::WORD_TYPE, word_type_list::DBL_OTHER);
    $result = num2bool($wrd_usr2->save());
    $target = true;
    test_dsp('word->save undo the user word fields beside the name for "' . word::TEST_NAME_CHANGED . '"', $target, $result, TIMEOUT_LIMIT_DB_MULTI);
    ...
    $result = $wrd_usr2_reloaded->plural;
    $target = word::TEST_NAME_CHANGED . 's';
    test_dsp('word->load plural for "' . word::TEST_NAME_CHANGED . '" unchanged now also for user 2', $target, $result);
    $result = $wrd_usr2_reloaded->description;
    $target = word::TEST_NAME_CHANGED . ' description';
    test_dsp('word->load description for "' . word::TEST_NAME_CHANGED . '" unchanged now also for user 2', $target, $result);
    $result = $wrd_usr2_reloaded->type_id;
    $target = cl(db_cl::WORD_TYPE, word_type_list::DBL_OTHER);
    test_dsp('word->load type_id for "' . word::TEST_NAME_CHANGED . '" unchanged now also for user 2', $target, $result);
