...
function create_test_view_components()
{
    test_header('Check if all base view components are existing');

    test_view_component(view_cmp::TN_TITLE, view_cmp_type::WORD_NAME);
    test_view_component(view_cmp::TN_VALUES, view_cmp_type::VALUES_ALL);
    test_view_component(view_cmp::TN_RESULTS, view_cmp_type::FORMULA_RESULTS);
    test_view_component(view_cmp::TN_TABLE, view_cmp_type::WORD_VALUE);
...
function run_view_component_test()
{
    ...
    */
    ...
    test_dsp('view_component->save adding logged for "' . view_cmp::TN_ADD . '"', $target, $result);
    ...
    $result = $cmp->save();
    $target = '';
    test_dsp('view_component->save rename "' . view_cmp::TN_ADD . '" to "' . view_cmp::TN_RENAMED . '".', $target, $result, TIMEOUT_LIMIT_DB_MULTI);
    ...
    $target = true;
    test_dsp('view_component->load renamed view_component "' . view_cmp::TN_RENAMED . '"', $target, $result);
    ...
    $target = '';
    test_dsp('view_component->save all view_component fields beside the name for "' . view_cmp::TN_RENAMED . '"', $target, $result, TIMEOUT_LIMIT_LONG);
    ...
    test_dsp('view_component->load type_id for "' . view_cmp::TN_RENAMED . '"', $target, $result);
    ...
    test_dsp('view_component->load comment for "' . view_cmp::TN_RENAMED . '" logged', $target, $result);
    $log->field = 'view_component_type_id';
    $result = $log->dsp_last(true);
    $target = 'zukunft.com system test added word name';
    test_dsp('view_component->load view_component_type_id for "' . view_cmp::TN_RENAMED . '" logged', $target, $result);
    ...
    $target = '';
    test_dsp('view_component->save all view_component fields for user 2 beside the name for "' . view_cmp::TN_RENAMED . '"', $target, $result, TIMEOUT_LIMIT_DB_MULTI);
    ...
    $result = $cmp_usr2->save();
    $target = '';
    test_dsp('view_component->save undo the user view_component fields beside the name for "' . view_cmp::TN_RENAMED . '"', $target, $result, TIMEOUT_LIMIT_DB_MULTI);
    ...
    //test_dsp('view_component->load type_id for "'.view_component::TEST_NAME_RENAMED.'"', $target, $result);
