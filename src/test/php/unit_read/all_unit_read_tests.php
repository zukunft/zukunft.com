...
class all_unit_read_tests extends all_unit_tests
{
    ...
    function run_unit_db_tests(all_tests $t): void
    {
        ...
        $this->init_unit_db_tests($t, $msg);
        $this->usr1->load_usr_data($msg);
    ...
    private function init_unit_db_tests(all_tests $t, user_message $msg): void
    {
        $t_db = new test_db_load($t);
        // add functional test rows to the database for read testing e.g. exclude sandbox entries
        $t_db->test_triple($msg,
        ...
        // the Euler group needs a name of its own, because the group name is unique and the
        // pi group above already carries groups::TN_READ
        $phr_grp = $t_db->add_phrase_group(array(triple_names::E), groups::TN_READ_E, $msg);
