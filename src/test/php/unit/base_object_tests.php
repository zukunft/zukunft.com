...
class base_object_tests
{
    function run(test_cleanup $t): void
    {
        ...
        $t->assert($test_name, $msg->text(), 'trying to add ""2020" (word_id 136) for user 3 (zukunft.com system test)" which is already part of the word list');
        ...
        $t->assert_true($test_name, $lst->has_id(word_names::MATH_ID));
        $test_name = 'check non existing id';
        $t->assert_false($test_name, $lst->has_id(word_names::YEAR_2026_ID));
        ...
        $t->assert($test_name, $lst->get(word_names::YEAR_2020_ID)->id(), word_names::YEAR_2020_ID);
        $test_name = 'get returns null if id is missing';
        $t->assert_null($test_name, $lst->get(word_names::YEAR_2021_ID));
        $test_name = 'first object is returned';
        $t->assert($test_name, $lst->get_first_object()->id(), word_names::MATH_ID);
        $test_name = 'diff can remove first two';
        $del_lst = new ListOfIdObjects([$t_wrd->word(), $t_wrd->word_inhabitant()]);
        $t->assert($test_name, $lst->diff($del_lst)->ids(), [word_names::ZH_ID,word_names::YEAR_2020_ID]);
        $test_name = 'diff can remove all duplicates';
        $lst = new ListOfIdObjects([$t_wrd->word(), $t_wrd->word_inhabitant(), $t_wrd->word_inhabitant()]);
        $del_lst = new ListOfIdObjects([$t_wrd->word_inhabitant()]);
        $lst = $lst->diff($del_lst);
        $t->assert($test_name, $lst->ids(), [word_names::MATH_ID]);
        $test_name = 'add object';
        $lst->add_obj($t_wrd->word_2020());
        ...
        $t->assert_true($test_name, $lst->has_name(word_names::MATH));
        $test_name = 'check non existing name';
        $t->assert_false($test_name, $lst->has_name(word_names::YEAR_2026));
        ...
        $t->assert($test_name, $lst->get_by_name(word_names::YEAR_2020)->id(), word_names::YEAR_2020_ID);
        $test_name = 'get returns null if name is missing';
        $t->assert_null($test_name, $lst->get_by_name(word_names::YEAR_2021));
        $test_name = 'id_by_name returns the id from object name';
        $t->assert($test_name, $lst->id_by_name(word_names::YEAR_2020), word_names::YEAR_2020_ID);
        $test_name = 'id_by_name returns zero if name is missing';
        $t->assert($test_name, $lst->id_by_name(word_names::YEAR_2021), 0);
        ...
        $lst->add_obj_by_name($wrd_no_id, $msg);
        $lst->add_obj_by_name($wrd2_no_id, $msg);
        $t->assert($test_name, 2, $lst->count());
        ...
        $lst->add_obj_by_name($wrd2_no_id, $msg, true);
        $t->assert($test_name, 3, $lst->count());
        ...
        $t->assert($test_name, $lst->names(), [word_names::MATH]);
        $test_name = 'unset by name can empty the list';
        $lst->unset_by_name(word_names::MATH);
        $t->assert_true($test_name, $lst->is_empty());
        ...
        $lst->add_obj_by_code_id($typ_no_id, $msg);
        $lst->add_obj_by_code_id($typ2_no_id, $msg);
        $t->assert($test_name, 2, $lst->count());
        $test_name = 'add_obj_by_code_id does not add object with same code_id but without id';
        $lst->add_obj_by_code_id($typ2_no_id, $msg);
        $t->assert($test_name, 2, $lst->count());
        $test_name = 'add_obj_by_name does not add object with same name but without id';
        $lst->add_obj_by_name($typ2_no_id, $msg);
        $t->assert($test_name, 2, $lst->count());
        ...
        $lst->add_obj_by_code_id($typ2_no_id, $msg, true);
        $t->assert($test_name, 3, $lst->count());
        $test_name = 'diff can remove all duplicates even if the id is missing';
