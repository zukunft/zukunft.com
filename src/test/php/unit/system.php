...
class system_unit_tests
{
    function run(test_cleanup $t): void
    {
        ...
        $val = $t->dummy_value();
        $target = '"Pi (math)" 3.1415926535898 (phrase_id_1 5) for user 1 (zukunft.com system test)';
        $t->assert($test_name, $val->dsp_id(), $target);
        $test_name = 'debug value_list id';
        $val_lst = $t->dummy_value_list();
        $target = '"Pi (math)" 3.1415926535898 / "inhabitant in the city of Zurich (2019)" 415367 (phrase_id_1 5,1640702804000) for user 1 (zukunft.com system test)';
        $t->assert($test_name, $val_lst->dsp_id(), $target);
        $test_name = 'debug value_phrase_link id';
        $val_lnk = $t->dummy_value_phrase_link();
        $target = 'link "Pi (math)" 3.1415926535898 (phrase_id_1 5) to "Mathematics" (word_id 1) as phrase for zukunft.com system test (1)';
        ...
        $target = 'value figure "Pi (math)" 3.1415926535898 (phrase_id_1 5) for user 1 (zukunft.com system test) 2022-12-26 18:23:45';
