...
class system_unit_tests
{
    function run(test_cleanup $t): void
    {
        ...
        $target = '"Mathematical constant","Mathematics","Pi","Pi (math)","constant" (-2,-1,1,2,3)';
        $t->assert($test_name, $phr_lst->dsp_id(), $target);
        $test_name = 'debug phrase_group id';
        $grp = $t->dummy_phrase_group();
        $target = '"Pi (math)" (1) as "Pi (math)" for user 1 (zukunft.com system test)';
