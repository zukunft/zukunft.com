  ...
  if ($usr->id > 0) {
    ...
    if ($wrd->id > 0) {
      ...
      if ($view_id > 0) {
        ...
        $result .= zu_err('No view for "'.$wrd->name.'" found.', "view.php", '', (new Exception)->getTraceAsString(), $usr);  
      }
      
    } else {
      $result .= zu_err("No word selected.", "view.php", '', (new Exception)->getTraceAsString(), $usr);  
    }
  }
