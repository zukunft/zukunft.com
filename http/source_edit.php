...
// open database
$db_con = zu_start("source_edit", "", $debug);
  ...
  if ($usr->id > 0) {
    ...
    // prepare the display
    $dsp = new view_dsp;
    $dsp->id = cl(SQL_VIEW_SOURCE_EDIT);
    $dsp->usr = $usr;
    $dsp->load($debug-1);
    ...
    if ($src->id <= 0) {
      $result .= zu_err("No source found to change because the id is missing.", "source_edit.php", '', (new Exception)->getTraceAsString(), $this->usr);
    } else {
