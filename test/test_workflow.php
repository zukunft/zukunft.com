...
/*
    ...
    test_api.php - run the internal api tests without unit, db read or write tests
    ------------
...
if ($db_con->is_open()) {
    ...
    if ($start_usr->id() > 0) {
        if ($start_usr->is_admin()) {
            ...
            // init tests
            $t = new all_tests();
            $t->header('Start zukunft.com api tests');
            $t->set_users();
            $usr_msg = new user_message();

            // run the api tests
            $t_api = new all_workflow_tests();
            $t_api->run_workflow_tests($t, $t->usr1, $usr_msg);
