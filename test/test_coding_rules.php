...
/*
    ...
    test_coding_rules.php - check if the code rules are used in all classes
    ---------------------

    check the length of the functions and classes and other coding rules
...
if ($db_con->is_open()) {
    ...
    if ($start_usr->id() > 0) {
        if ($start_usr->is_admin()) {
            ...
            // init tests
            $t = new all_tests();
            $t->header('Start zukunft.com coding rule check');
            $t->set_users();
            $usr_msg = new user_message();

            // check the coding rules
            new coding_rule_tests()->run($t);
            ...
            echo 'Only admin users are allowed to perform the coding rule check. Login as an admin for system testing.' . "\n";
        }
    }
