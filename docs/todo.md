...
A proper issue ticket should be created for these TODOs notes:
    ...
    TODO review cache handling
         for unit tests    only the initial calling user is verified via db
                           all unit test data are generated by the create_test_data class
    ...
    TODO move funtionality from the deprecated non view.php http calls the the system views
    ...
    TODO cleanup init.php to speed up the api
         ...
         reduce the number of callable scripts to view.php, api and test
    TODO review include so that it is one directional and remove in parent objects the include of a child object
                        so the child includes the parent but the perant never a child
