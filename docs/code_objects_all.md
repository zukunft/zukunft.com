...
│           │   ├── element - $elm - either a word, triple, verb or formula with a link to a formula
...
│           │   │   │   ├── sandbox_link_named
│           │   │   │   │   └── triple - $trp - the object that links two words (an RDF triple)
...
│           │   │   │   └── view_relation - $mrl - to define the relation between two views e.g. to have a parent view where the child view have additional components
│           │   │   └── sandbox_named
│           │   │       └── sandbox_typed
│           │   │           └── sandbox_code_id
│           │   │               ├── component - $cmp - a single display object like a headline or a table
│           │   │               ├── formula_map
│           │   │               │   └── formula - $frm - the main formula object for calculation
│           │   │               ├── source - $src - the source object to define a source for values
│           │   │               ├── view - $msk - the main display object
│           │   │               └── word - $wrd - the main word object
│           │   └── job - $job - object to combine all parameters for one calculation or cleanup request
│           ├── type_object
...
│           │   ├── language - $lan - to define a language for the user interface
...
│           │   ├── verb - $vrb - predicate object to link two words
...
│   │           └── sandbox_multi
│   │               ├── group - $grp - a combination of a word list and a triple_list
│   │               └── sandbox_value
│   │                   ├── result
│   │                   │   └── result_two - overwrite the result functions for up to two assigned triples
│   │                   ├── value_base
│   │                   │   ├── value - $val - the main numeric value object using the prime, norm and big value keys
...
│       │   │   └── view_relationUi - $mrl - create HTML code to display a n:m link between two views
│       │   ├── sandbox_namedUi
│       │   │   ├── groupUi - $grp - the extension of the phrase group api object to create the HTML code to display a word or triple
│       │   │   ├── sandbox_typedUi
│       │   │   │   └── sandbox_code_idUi
│       │   │   │       ├── componentUi
...
│       │   │   │       ├── formulaUi - $frm - the display extension of the api formula object
│       │   │   │       ├── sourceUi - $src - the extension of the source API objects to create source base html code
│       │   │   │       ├── view_baseUi
│       │   │   │       │   └── view_exeUi
│       │   │   │       │       └── viewUi - $msk - the main frontend object to create the html code
│       │   │   │       ├── tripleUi - $trp - create the HTML code to display a triple (two linked words or triples)
│       │   │   │       └── wordUi - $wrd - create HTML code to display a words based on the api json message
│       │   │   └── verbUi - $vrb - the display extension of the api verb object
│       │   └── sandbox_valueUi
