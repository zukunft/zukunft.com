...
- In a test the `user_message $msg` is created **once** at the block's init (like `$test_name`) and reused for every call; `$msg->reset()` is **test-only** and used **only right after a test that is meant to fail** — to clear the expected error before the next assertion. Production code never resets `$msg` (it is append-only; use a local buffer + `merge()`). → `docs/llm/testing.md`
