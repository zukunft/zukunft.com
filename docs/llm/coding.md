...
- Outside tests a `user_message` is created **once per request, at the http / api entry point**; every `new user_message(` below an entry point is an exception that needs a comment saying why (a buffer that is merged back, a message of another user). The generated `docs/code_user_message_exceptions.md` lists every still-unexplained one and must shrink, never grow. → `docs/llm/state-and-messages.md`
