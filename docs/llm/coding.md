...
- An object without its own db id can be normal, not invalid: a list holds what is not yet written, so membership and duplicate detection go by the object's key (the name, the linked objects of a link, or a generated key), never by its own id. `can_be_ready()` = may be held in a list, `db_ready()` = every *linked* object has an id so the row can be written now (its own id is not required, the insert assigns it), `is_valid()` / `id() != 0` = already written. In an import "not ready yet" is a normal intermediate state and only an error if it survives the last pass. → `docs/llm/architecture.md`
- A list is not automatically a set: a view uses the same component several times and a verb list counts the verb usages of a triple list, so whether a repeat is a double or the data is the **caller's** decision, passed as `$allow_duplicates` to `add_obj` — `false` refuses and reports `LIST_DOUBLE_ENTRY`, `true` adds it and reports nothing; never hard-code the check in the list class and test both branches. → `docs/llm/architecture.md`
...
- A generated file is never edited by hand and never by the LLM: every file that says `do not edit manually` (`docs/code_object_name_exceptions.md`, `docs/code_user_message_exceptions.md`, `docs/code_test_coverage.md`, `docs/json_findings.md`, `docs/code_functions_all.md`) and every regenerated baseline (`list.csv`, the html snapshots, the api fixtures) is rewritten by the test run with `AUTO_UPDATE_TEST_FILES`. A test that reports such a file as outdated is a signal about the code or the data that produced the difference, not a request to edit the file: fix the cause if there is one, otherwise leave the file to the next run. → `docs/llm/testing.md`
...
- Outside tests a `user_message` is created **once per request, at the http / api entry point**; every `new user_message(` below an entry point is an exception that needs a comment saying why (a buffer that is merged back, a message of another user), written **behind the creation on the same line**, not above it. The generated `docs/code_user_message_exceptions.md` lists every still-unexplained one and must shrink, never grow. → `docs/llm/state-and-messages.md`
- A created `user_message` never gets lost: what it collects reaches the caller — merged (`$msg->merge($sub_msg)`), returned, read (`is_ok()`), or kept in an object field. A message that is only filled and then goes out of scope drops every error it collected; worst is the nameless `$obj->set_type_id($id, new user_message($usr))`, created only to fill a required parameter — thread the caller's `$msg` instead. An intended drop (property hook, deprecated display path, the log writer) says `not reported` in the comment behind the creation; the generated `docs/code_user_message_exceptions.md` lists every still-lost message and must shrink, never grow. → `docs/llm/state-and-messages.md`
- A `user_message` parameter is **required** — never `?user_message $msg = null` and never `user_message $msg = new user_message()`. Both drop what the function reports; the nullable form only writes the drop out honestly, and `$msg?->add(...)` reports nothing at the call sites that matter. A request has exactly one message, so `null` describes a state that does not exist. A caller with no message gets one itself (threaded from its caller, or created there with the trailing comment if it is an entry point). If php complains about a required parameter after an optional one, reorder; if an override cannot add the parameter, the base class and every child take it in one commit. → `docs/llm/state-and-messages.md`
- A function that only *builds and returns* a `user_message` (`(...): user_message` with a fresh message inside) takes `user_message $msg` as a parameter instead and returns `bool` (`$msg->is_ok()`); "the message is my return value" is the most common excuse for a creation below the entry point, and every caller that dropped the return was dropping errors. Before threading, check per call site *which* message belongs there (a per-level import buffer, not the request message) and whether the callee's reporting is wanted at all — pass the existing `report_missing`-style flag rather than surfacing a diagnostic the caller already reports. → `docs/llm/state-and-messages.md`
- The http entry point (`http/view.php`) is the only place that determines the requesting user; it sets `$msg->usr` as early as possible (before the blocked-request branch and the cached-page fast path) and every function below takes `$msg` as a parameter and reads the requesting user from `$msg->usr` — never a second user parameter, never a global, never `$_SESSION`. A user the function operates *on* (`take_ownership`, `no_diff`) stays its own parameter. → `docs/llm/state-and-messages.md`
- A step whose errors would only be a **consequence** of an earlier error is guarded with `if (<the message of the step it depends on>->is_ok())`, so the user sees only errors that are certainly real; a step with **independent** errors is never guarded. Gate on a message scoped to the step depended on (not the request message, which would suppress unrelated errors), keep that message free of normal-path notices (`add(..., ok: true)` informs without suppressing), and say in the `else` branch that the step was skipped. → `docs/llm/dependent-errors.md`
...
- `u` (`url_var::USER`) in a page or api url is always the logged-in user and is added centrally (`html_base::ref` / `form_user`, `frontend::redirect_url`, `rest_call::data_with_user`), never by a url builder; a user that is shown or changed is named by `ue` (`url_var::USER_TO_EDIT`), which the views of `views::USER_TO_EDIT_MASKS_IDS` use instead of `id` (`url_var::id_var`); the session, never the url, decides who the user is. → `docs/llm/state-and-messages.md`
...
- The code id of an object is only *shown* to an admin or a developer (`can_see_code_id`) and only *editable* for a profile that passes `can_set_code_id`; every other profile — including the system and test users — never sees it, so the rendered snapshots stay free of the field. → `docs/llm/state-and-messages.md`
...
- Behaviour that describes a phrase rather than a word (parents, children, similar, ...) lives once on `web/phrase/phrase.php`; `web/word/word.php` and `web/word/triple.php` keep a one-line delegate each, so a triple page can do everything a word page can. Never push it up into `sandbox_code_id` (views/formulas/components have no `phrase()`) and never use a trait (the project has none). A page-object fixture stuck on the wrong class is the symptom of a missing delegate. → `docs/llm/frontend.md`
...
- Every frontend list has three versions — short (5 entries), more (20) and all (paged, prev/next, from the screen cache, only below the 2'000 max frontend list size); both counts come from `config.yaml` via `$ui_sys->cfg`, never as a literal. → `docs/llm/frontend.md`
- A page must not fill the screen, because the user messages are shown below the view and would else be hidden below the fold; so every list is limited **on its own** — each group of a grouped list gets the configured limit and its own "… and n more", not only the ungrouped rest. → `docs/llm/frontend.md`
- A truncated list's "… and n more" tail is always a link to the next version of the list (short → more → all, e.g. the values tail links to the `phrase_values` view), never dead text; `n` is the number of extra items. → `docs/llm/frontend.md`
...
- Keep a change to an import json as small as the task: fix the named entry only — never reformat, re-indent or re-order a file, and never add or remove a word, triple, view, component, formula or source that the task did not ask for, because every insert or delete shifts the db id of all later objects and breaks the pinned `*_ID` consts and the `list.csv` baselines. If the fix cannot avoid it, report the id shift instead of absorbing it silently. → `docs/llm/json_structure.md`
...
- An unnamed triple *is* its auto-generated name — `<from> (<to>)` for the `is` verb, `<from> <verb> <to>` for every other one: nothing can reference it by a shorter phrase, so give it the explicit `name` a value or formula needs. → `docs/llm/json_structure.md`
- A use case file (`files::USE_CASE_FILES`, `messages/use_cases/`) **never** carries a `code_id` — not on a word, triple, formula, view or component — and must always import for a **normal** user: a use case is user data that any user may add, while a `code_id` marks system data that only a system or developer user may set. A borrowed system triple is repeated **without** its `code_id`; the import merges it by name into the existing row, which keeps the code id it has. The unit test of the file imports it as a normal user, so a code id fails the test. Every row of a use case — the view, the phrase of the page, the values — is selected by its **name**, never by a code id or a database id (the id follows the import order): tests use `assert_view_by_name` and a name const without an `_ID` twin. → `docs/llm/json_structure.md`
- Word and triple share one phrase namespace: never declare the same name as both (qualify the triple, e.g. the word `km` plus the triple `km (unit symbol)`) — the classic leftover after splitting a multi-word word. → `docs/llm/json_structure.md`
- A source name belongs in `value.source`, never in the value's `words`; the phrase group names the claim the number is about. → `docs/llm/json_structure.md`
- A borrowed triple cannot be re-declared name-only — repeat the home file's `from`/`verb`/`to` verbatim, or the last import silently wins. → `docs/llm/json_structure.md`
...
- The home file of a phrase is the one imported **first** (`cfg/const/files.php`: `SYSTEM_DATA_FILES` → `BASE_DATA_FILES` → `BASE_DATA_PATH_FILES`), so `solution_prio.json` owns `probability`/`score`/`return` and `math`/`economics` re-declare them name-only; a growing domain file is imported last to keep the test-pinned ids stable. → `docs/llm/json_structure.md`
...
- Never add a `measured value` qualifier (to a value, to a calc-validation `context`, or as a word/triple): every value is assumed to be measured. Mark only the deviation with the word `assumed` (never an `assumed value` triple, because the phrases of a value are its words) and name a `source` for the measured ones. → `docs/llm/json_structure.md`
...
- The full load (`import_base_data` / `import_test_data` of `test/test_full_load.php`) imports with the normal test user, so no file of its lists may carry a `code_id` or a `ui_msg_code_id`; that data needs a system user and belongs in `files::SYSTEM_DATA_FILES`. → `docs/llm/testing.md`
- A phrase fixture sets `set(<id>, <name>)`, never `set_name()` alone: `combine_object::api_json_array` emits `OBJECT_CLASS` only for a non-zero id, so an id-less phrase nested as a triple's `from`/`to` arrives empty at the frontend and renders as `<a href="…"></a>`. → `docs/llm/testing.md`
- When a split turns a word into a triple the fixture changes class with it — const from `word_names` to `triple_names`, factory from `test_words` to `test_triples`; a word-typed stand-in carrying the right name is not a substitute. → `docs/llm/testing.md`
- An unnamed import triple is pinned by its **generated** name, which is `<from> (<to>)` for the `is` verb and `<from> <verb> <to>` for every other one; read it out of `list.csv` next to the id instead of composing it. → `docs/llm/testing.md`
- A `*_COM` description const is the owning import file's text pasted verbatim; a shorter hand-written summary makes the unit test and the database read disagree. → `docs/llm/testing.md`
- A phrase id const (`word_names`, `triple_names`, …) is re-baselined from the regenerated `src/test/resources/unit/<class>/list.csv` after a reset, never guessed: adding or removing a phrase in a seed import shifts every later id, and `diff_msg` compares keys only, so the reported drift understates the real one. → `docs/llm/testing.md`
- A fixture mismatch is fixed by first listing **every** producer of that fixture (in-memory factory, import data → db row, ui round trip) and making them agree at the source — never by patching the side the current error report shows; the same test failing twice with the diff direction inverted (`//+x//` then `//-x//`) is the signature of two disagreeing producers, not of a half-applied fix. → `docs/llm/testing.md`
- All test objects come from a `create/test_*.php` factory — single objects and populated lists alike, never inline construction.
- A test url array is built from the `url_var` keys and the consts of the test object (e.g. `[url_var::ID => word_names::MATH_ID, url_var::NAME => word_names::MATH]`), never with `to_url_array($msg)`, so one test checks the url contract and the mapping together and one url checks several fields; adjusting such tests when a key or const changes is accepted, and `to_url_array` is only called in its own round-trip test. → `docs/llm/testing.md`
...
- A test object is created by the normal user `$t->usr1` (the standard owner); use `$t->usr2` only to test a second user's sandbox and `$t->usr_admin` only when the create needs an admin-only privilege (not `set_code_id` — `$t->usr1`'s test profile already passes `can_set_code_id()`); never create a test object as `$t->usr_system` (the system user is for system-level functions, and system-owned test data cannot be removed by the normal-user cleanup). The carve-out is *permanent* fixture data that is never cleaned up (the year words, like the imported fundamentals) — that may be system-owned, as a narrow commented exception. → `docs/llm/testing.md`
...
- In a test the `user_message $msg` is created **once** at the block's init (like `$test_name`) and reused for every call; `$msg->reset()` is **test-only** and belongs **after every assert that has checked the result** — a test run should find as many independent issues as possible, and a message still carrying the previous reports both hides the next failure and suppresses the work of every function that gates on `$msg->is_ok()`. Production code never resets `$msg` (it is append-only; use a local buffer + `merge()`). → `docs/llm/testing.md`
...
- In a horizontal round-trip test a field that no form transports is backfilled from the original only when the refilled value is null, so the diff still catches a mapper that fabricates or distorts the field. → `docs/llm/testing.md`
- A html snapshot page (`src/test/resources/web/html/`) gets its styles and icon fonts from the local pod at `http://localhost/`; the icon glyphs additionally need the font CORS header of `external_lib/.htaccess`, because `@font-face` is CORS-gated. → `docs/llm/testing.md`
