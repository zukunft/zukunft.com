...
```
http://localhost/http/view.php?m=1&debug=6
```
...
level* — from `&debug=10` upward (`DEBUG_LEVEL_MAX_FIXED + 1`) — so the plain
...
```php
// show every db write from '&debug=5' upward
...
only from `&debug=10` upward (`DEBUG_LEVEL_MAX_FIXED + 1`), i.e. in the depth
...
| `DEBUG_LEVEL_DB_WRITE` | `5` | write actions to the database |
| `DEBUG_LEVEL_DB_READ` | `6` | read actions from the database |
| `DEBUG_LEVEL_COMPLEX_FUNCTION` | `7` | potentially long-running function calls, e.g. a suspected endless loop |
| `DEBUG_LEVEL_MAX_FIXED` | `9` | the last predefined level and the start of the *depth* debug levels: from `9` upward each higher number drills one more call deeper |

The ordering runs from the rarest, most significant activity (external calls at
`1`) to the most frequent, most voluminous (database reads at `6`), then the
long-runner tracing at `7`; from `9` upward the depth levels drill one call deeper
for each additional number, for tracing a specific deep call chain.

## Typical use

- `?debug=1` — is the request reaching out to an external system at all?
- `?debug=6` — see every SQL read (and everything above it) a request triggers,
  e.g. to check how many database reads a cached vs. an uncached page needs.
- `?debug=7` — hunt a suspected endless loop or a slow function.
