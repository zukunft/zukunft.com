...
the confirm add formula link view e.g. http://localhost/http/view.php?token=8xxx1&m=45&z=1&p=274&f=39&submit=link should not show the phrase_id and the formula_id in the change table. Instead above the table should be shown a text line with in bold with something like 'link formula xxx to phrase yyy'. In the table with the changed fields instead of the type id or any other id the type name or any other name should be shown. 
