...
BLOCKER: the /.git folder is served from the web root, so the full source and the whole commit history can be downloaded. script/install.sh line 239 does 'git clone ... "$WWW_ROOT/"' and .htaccess has no rule for .git (the dotfile rule '<FilesMatch "^\.">' matches only the last path segment, so /.git/config, /.git/HEAD and /.git/objects/... are served). Deploy an exported tree instead of a live clone (or move the docroot to a public subfolder) and add an explicit 'RedirectMatch 404 ^/\.git' as a second line of defence
