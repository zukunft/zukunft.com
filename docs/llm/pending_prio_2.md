...
2. set_type family (permission-checked type setter, same shape): sandbox_typed.php:219 set_type_id, :287 set_type, :306 set_type_by_code_id, :340 set_type_by_name; sandbox_link_named.php:365 set_type_id, :399 set_type_by_code_id, :433 set_type_by_name; job.php:282 set_type_id, :311 set_status_id. NOTE set_type_id has ~29 callers — the biggest of these; set_type_by_code_id/by_name have 0 direct callers (reached via set_type). the set_type_*_db (no-check) variants are the analog of set_code_id_db for the load/factory callers.
3. fill family (fill(obj, user $usr_req): user_message): sandbox_code_id.php:462, db_object_seq_id.php:589, db_object_multi.php:279, helper/type_object.php:821, phrase.php:802. CORE method with many callers and a deep merge chain — bigger and riskier; evaluate whether it should return bool or keep returning user_message (a leaf that the caller merges may be legitimate) before touching it.
4. evaluate, do NOT assume a refactor (these may legitimately RETURN a user_message as the result the caller merges, the sanctioned leaf pattern from the audit): import_file.php:95 json_file, :194 yaml_file, :228 import_config_yaml (import entry points); user.php:3125 db_insert, :3173 db_update_user, user_list.php:647 add, sql_db.php:5957 create_internal_words, db_id_object_non_sandbox.php:227 del_exe (save/setup leaves). Decide per function whether the caller already threads a $msg (→ refactor) or the returned message is the function's result (→ leave).
